	...
	implements ActionListener
{
	...
	private GridBagLayout CashBookOnlyLayout = new GridBagLayout();
	private CPanel CashBookOnlyPanel = new CPanel();
	private CComboBox CashBookOnlyCombo = new CComboBox();
	private CLabel CashBookOnlyLabel = new CLabel();
	private CLabel CashBookOnlyDummyLabel = new CLabel();
	/**
	 *	Static Init
	 *  @throws Exception
	 */
	private void jbInit() throws Exception
	{
		...
		CashBookOnlyPanel.setLayout(CashBookOnlyLayout);
		CashBookOnlyLabel.setText(Msg.translate(Env.getCtx(), "C_CashBook_ID"));
		CashBookOnlyDummyLabel.setText("");
		mainPanel.add(CashBookOnlyPanel, BorderLayout.EAST);
		CashBookOnlyPanel.add(CashBookOnlyLabel,  new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0
				,GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(2, 0, 2, 0), 0, 0));
		CashBookOnlyPanel.add(CashBookOnlyCombo,  new GridBagConstraints(1, 1, 2, 1, 0.0, 0.0
				,GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(2, 5, 2, 5), 0, 0));
		CashBookOnlyPanel.add(CashBookOnlyDummyLabel,  new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0
				,GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(2, 0, 30, 0), 0, 0));
	...
	private boolean dynInit (VButton button) throws Exception
	{
		...
		CashBookOnlyPanel.setVisible(m_onlyRule);
		...
		m_cashLine = null;
		m_C_CashLine_ID = 0;
		Object cashBook = m_mTab.getValue("C_CashBook_ID");
		if(cashBook == null)
			m_C_CashBook_ID = 0;
		else
			m_C_CashBook_ID = (Integer)cashBook;
		 ...
		 */
		SQL = MRole.getDefault().addAccessSQL(
			"SELECT C_CashBook_ID, Name, AD_Org_ID, IsDefault FROM C_CashBook WHERE IsActive='Y' ORDER BY Name",
		...
		try
		{
			...
			while (rs.next())
			{
				...
				CashBookOnlyCombo.addItem(pp);
				if (key == m_C_CashBook_ID)
					kp = pp;
				if (kp == null && rs.getInt(3) == m_AD_Org_ID && rs.getString(4).equals("Y"))	//  Default Org
					kp = pp;
			}
		}
		...
		if (kp != null)
		{
			...
			CashBookOnlyCombo.setSelectedItem(kp);
		...
		else
		{
			try
			{
				...
				while (rs.next())
				{
					...
					String name = rs.getString(2);
					KeyNamePair pp = new KeyNamePair(key, name);
					if (kp == null && rs.getInt(3) == m_AD_Org_ID )	
						kp = pp;
				}
			}
			catch (SQLException epc)
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if (e.getSource() == paymentCombo)
		{
			...
			if (pp != null)
			{
				...
				if(this.m_onlyRule)
				{
					if(s.equals(X_C_Order.PAYMENTRULE_Cash.toLowerCase()))
					{	
						CashBookOnlyLabel.setVisible(true);
						CashBookOnlyCombo.setVisible(true);
						CashBookOnlyDummyLabel.setVisible(false);
					}
					else
					{	
						CashBookOnlyLabel.setVisible(false);
						CashBookOnlyCombo.setVisible(false);
						CashBookOnlyDummyLabel.setVisible(true);
					}
	...
	private boolean saveChanges()
	{
		...
		if (m_onlyRule)
		{
			...
			{
				KeyNamePair kp = (KeyNamePair)CashBookOnlyCombo.getSelectedItem();
				if (kp != null)
					m_mTab.setValue("C_CashBook_ID", kp.getKey());
			}
			else
				m_mTab.setValue("C_CashBook_ID", null);
		...
		if (!newPaymentRule.equals(m_PaymentRule))
		{
			...
			else if ("KTSD".indexOf(m_PaymentRule) != -1 && "KTSD".indexOf(newPaymentRule) != -1 && m_mPaymentOriginal != null)
			{
				...
				boolean ok = DocumentEngine.processIt(m_mPaymentOriginal, DocActionConstants.ACTION_Reverse_Correct);
		...
		if (newPaymentRule.equals(X_C_Order.PAYMENTRULE_Cash))
		{
			...
			if (C_Invoice_ID == 0 && order == null)
			{
				...
				ADialog.error(m_WindowNo, this, "PaymentError", Msg.translate(Env.getAD_Language(Env.getCtx()),
						"CashNotCreatedNoInvoice"));
			}
			else
			{
				//  Changed Amount
		...
		if ("KS".indexOf(newPaymentRule) != -1)
		{
			...
			{
				boolean ok = DocumentEngine.processIt(m_mPayment, DocActionConstants.ACTION_Complete);
				m_mPayment.save();
				if (ok)
