...
public final class VBPartner extends CDialog implements ActionListener
{
	...
	private boolean actionSave()
	{
		...
		if (m_partner == null)
		{
			...
			m_partner = MBPartner.getTemplate(ctx, AD_Client_ID);
			
			//SR 10020089: Get organization from parent window while creating business partner
			int windowOrgId = ctx.getAD_Org_ID(m_WindowNo);
			if (ctx.getAD_Org_ID() != windowOrgId && windowOrgId > 0)
				m_partner.setAD_Org_ID(windowOrgId);
			else
				m_partner.setAD_Org_ID(ctx.getAD_Org_ID());
