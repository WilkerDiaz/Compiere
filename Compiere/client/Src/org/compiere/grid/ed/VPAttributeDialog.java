...
implements ActionListener
{
	...
	private void jbInit () throws Exception
	{
		...
		verticalPane = new JScrollPane(centerPanel,
	...
	private void addAttributeLine (MAttribute attribute, boolean product, boolean readOnly)
	{
		...
		CLabel label = new CLabel (attribute.getName(Env.getAD_Language(Env.getCtx())));
		if (product)
		...
		if (X_M_Attribute.ATTRIBUTEVALUETYPE_List.equals(attribute.getAttributeValueType()))
		{
			...
			for (MAttributeValue value:values)
				if (value!=null) value.setLanguage(Env.getAD_Language(Env.getCtx()));
	...
	{
		log.config("");
		int M_Locator_ID=Env.getCtx().getContextAsInt(m_WindowNoParent, "M_Locator_ID", true);
	...
	private boolean saveSelection()
	{
		...
		if(m_productWindow)
		{
			...
			if(attributes != null && attributes.length > 0){
				...
				String sql = "SELECT M_AttributeSetInstance_ID , COUNT(1) FROM M_AttributeInstance WHERE (";

				boolean first = true;
				String attrList = " AND M_Attribute_ID IN ( ";
				List<Integer> attrParams = new ArrayList<Integer>();
				for(int i=0; i<attributes.length; i++) {
					...
					attrParams.add(attributes[i].getM_Attribute_ID());
					sql += " (M_Attribute_ID=? ";
					params.add(attributes[i].getM_Attribute_ID());
					if(X_M_Attribute.ATTRIBUTEVALUETYPE_List.equals(attributes[i].getAttributeValueType())) {
						sql += " AND M_AttributeValue_ID=? ) ";
						CComboBox editor = (CComboBox)m_editors.get(i);
						params.add(((MAttributeValue)editor.getSelectedItem()).getM_AttributeValue_ID());
					}
						...
						params.add(editor.getValue());
					}
					else {
						sql += " AND Value=? ) ";
						VString editor = (VString)m_editors.get(i);
						params.add(editor.getText());
					}
				}
				attrList += " ) ";
				sql += " ) "+ attrList;
				params.addAll(attrParams);

				sql += " GROUP BY M_AttributeSetInstance_ID HAVING COUNT(1)= ? ";
				params.add(attributes.length);

				Object[][] results = QueryUtil.executeQuery((Trx)null, sql, params.toArray());
				boolean found = false;
				if(results!=null && results.length > 0 && results[0].length > 0) {
					for(Object[] o : results) {
						MAttributeSetInstance asi = MAttributeSetInstance.get(Env.getCtx(), ((BigDecimal)o[0]).intValue(), m_M_Product_ID);
						if(asi.getM_AttributeSet_ID() == as.getM_AttributeSet_ID()){
							found = true;
							m_M_AttributeSetInstance_ID = ((BigDecimal)o[0]).intValue();
							break;
						}
					}
				}
				...
				else {	// Create new M_AttributeSetInstance
					m_masi = new MAttributeSetInstance(Env.getCtx(), 0, as.getM_AttributeSet_ID(), (Trx)null);
					m_masi.save();
					m_M_AttributeSetInstance_ID = m_masi.getM_AttributeSetInstance_ID();
				}
			}
