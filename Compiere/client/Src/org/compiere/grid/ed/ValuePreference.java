	...
	implements ActionListener
{
	...
	private int getAD_Preference_ID()
	{
		...
		params.add(cbClient.isSelected() ? m_AD_Client_ID : 0);
		//	Org
		sql.append(" AND AD_Org_ID=?");
		params.add(cbOrg.isSelected() ? m_AD_Org_ID : 0);
		//	Optional User
		if (cbUser.isSelected()) {
			sql.append(" AND AD_User_ID=?");
			params.add(m_AD_User_ID);
		}
		else {
		...
		//	Optional Window
		if (cbWindow.isSelected()) {
			sql.append(" AND AD_Window_ID=?");
			params.add(m_AD_Window_ID);
		}
		else
			sql.append(" AND AD_Window_ID IS NULL");
		//	Attribute (Key)
		sql.append(" AND Attribute=?");
		params.add(m_Attribute);
		//
		log.finest(sql.toString());
		int id = QueryUtil.getSQLValue(null, sql.toString(), params.toArray());
		if (id < 0)
			id = 0;
		return id;
	}	//	getAD_Preference_ID
