	...
	implements ActionListener
{
	...
	private final VDate dateScheduledStart = new VDate(GlobalMessageConstants.SCHEDULED_START_DATE, false, false, true,
			DisplayTypeConstants.DateTime, Msg.getMsg(Env.getCtx(), GlobalMessageConstants.SCHEDULED_START_DATE, true));
	...
	void jbInit() throws Exception
	{
		...
		confirmPanel.addComponent(new JLabel(Msg.getMsg(Env.getCtx(), GlobalMessageConstants.SCHEDULED_START_DATE, true)));
		confirmPanel.addComponent(dateScheduledStart);
	...
	private void dynInit(int Record_ID)
	{
		...
		else if (m_AD_Table_ID == X_C_AllocationHdr.Table_ID)
		{
			//	Complete                    ..  CO
			if (DocStatus.equals(org.compiere.vos.DocActionConstants.STATUS_Completed) &&
					!isCashTypeAllocation(Record_ID))
			{
				options[index++] = org.compiere.vos.DocActionConstants.ACTION_Void;
				options[index++] = org.compiere.vos.DocActionConstants.ACTION_Reverse_Correct;
			}
		 ...
		 */		
				...
				|| m_AD_Table_ID == X_M_TaskList.Table_ID)
		{
			//	Draft                       ..  DR/IP/IN
			if (DocStatus.equals(org.compiere.vos.DocActionConstants.STATUS_Drafted)
				|| DocStatus.equals(org.compiere.vos.DocActionConstants.STATUS_InProgress)
	...
	private boolean checkStatus (String TableName, int Record_ID, String DocStatus)
	{
		...
		int result = QueryUtil.getSQLValue(null, sql);
		return result == 2;
	}	//	checkStatusChange
	...
	{
		m_batch = dateScheduledStart.getTimestamp() != null;
	...
	public Timestamp getDateScheduledStart() {
		return dateScheduledStart.getTimestamp();
	}
	
	public boolean isCashTypeAllocation(int Record_ID)
	{
		if(Record_ID == 0 || m_AD_Table_ID != X_C_AllocationHdr.Table_ID)
			return false;
		MAllocationHdr alloc = new MAllocationHdr(Env.getCtx(),Record_ID,(Trx)null);
		if(alloc != null)
		{
