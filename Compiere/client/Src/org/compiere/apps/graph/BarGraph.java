...
public class BarGraph extends CPanel implements ActionListener {
	...
	public BarGraph(MGoal goal) {
		...
		loadData();
		/**
		 * test add (new BarGraphColumn("Column 1", 100)); add (new
		 * BarGraphColumn("Column 2", 200)); add (new BarGraphColumn("Column 3",
		 * 300)); add (new BarGraphColumn("Column 4", 400)); add (new
		 * BarGraphColumn("Column 5", 500)); add (new BarGraphColumn("Column 6",
		 * 400)); add (new BarGraphColumn("Column 7", 300)); add (new
		 * BarGraphColumn("Column 8", 200)); add (new BarGraphColumn("Column 9",
		 * 100)); add (new BarGraphColumn("Column 10", 200)); add (new
		 * BarGraphColumn("Column 11", 300)); add (new BarGraphColumn("Column
		 * 12", 400)); add (new BarGraphColumn("Column 13", 500)); add (new
		 * BarGraphColumn("Column 14", 100));
		 */
	} // BarGraph
	...
	private void loadData() {
		...
		if (X_PA_Measure.MEASURETYPE_Calculated
				.equals(measure.getMeasureType())) {
			MMeasureCalc mc = MMeasureCalc.get(Env.getCtx(), measure
					.getPA_MeasureCalc_ID());
			String sql = mc.getSqlBarChart(m_goal.getRestrictions(false),
					m_goal.getMeasureDisplay(), m_goal.getDateFrom(), m_goal
							.getDateTo(), MRole.getDefault()); // logged in
																// role
			...
			} else // MMeasure.MEASUREDATATYPE_QtyAmountInTime
			{
				...
				if (X_PA_Goal.MEASUREDISPLAY_Year.equals(MeasureDisplay))
					trunc = "Y";
				else if (X_PA_Goal.MEASUREDISPLAY_Quarter
						.equals(MeasureDisplay))
					trunc = "Q";
		...
		// Request
		else if (X_PA_Measure.MEASURETYPE_Request.equals(measure
				.getMeasureType())) {
			MRequestType rt = MRequestType.get(Env.getCtx(), measure
					.getR_RequestType_ID());
			...
			try {
				...
				while (rs.next()) {
					...
					BarGraphColumn bgc = new BarGraphColumn(rt, data,
							R_Status_ID);
					if (R_Status_ID == 0) {
						Timestamp date = rs.getTimestamp(2);
						bgc.setLabel(date, m_goal.getMeasureDisplay());
		...
		// Project
		else if (X_PA_Measure.MEASURETYPE_Project.equals(measure
				.getMeasureType())) {
			MProjectType pt = MProjectType.get(Env.getCtx(), measure
					.getC_ProjectType_ID());
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() instanceof BarGraphColumn) {
			...
			if (bgc.getAchievement() != null) // Single Achievement
			{
				...
				query = Query.getEqualQuery("PA_Measure_ID", a
						.getPA_Measure_ID());
			} else if (bgc.getGoal() != null) // Multiple Achievements
			{
				MGoal goal = bgc.getGoal();
				query = Query.getEqualQuery("PA_Measure_ID", goal
						.getPA_Measure_ID());
			} else if (bgc.getMeasureCalc() != null) // Document
			{
				MMeasureCalc mc = bgc.getMeasureCalc();
				query = mc
						.getQuery(m_goal.getRestrictions(false), bgc
								.getMeasureDisplay(), bgc.getDate(), MRole
