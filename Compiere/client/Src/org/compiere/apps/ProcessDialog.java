	...
	public boolean init()
	{
		...
		try
		{
			...
			if (rs.next())
			{
				...
				//
				String isSOTrx = rs.getString(6);
				if(isSOTrx== null)
					isSOTrx="Y";
				
				Env.getCtx().setContext(m_WindowNo, "IsSOTrx", isSOTrx);
	...
	private void printInvoices()
	{
		...
		do
		{
			...
			for (int AD_Invoice_ID : m_ids) {
				ReportEngine re = ReportCtl.startDocumentPrint( Env.getCtx(), ReportEngine.INVOICE, AD_Invoice_ID, true );
				if (re == null) {
