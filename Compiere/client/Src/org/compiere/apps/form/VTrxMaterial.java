			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						{
							lineID = Env.getCtx().getContextAsInt(m_WindowNo, "M_WorkOrderTransactionLine_ID");
							if(lineID != 0)
							{
								log.fine("M_WorkOrderTransactionLine_ID="+lineID);
								AD_Window_ID = ZoomTarget.getZoomAD_Window_ID("M_WorkOrderTransaction", 0, "", true);
								ColumnName = "M_WorkOrderTransaction_ID";
								SQL = "SELECT M_WorkOrderTransaction_ID FROM M_WorkOrderTransactionLine WHERE M_WorkOrderTransactionLine_ID=?";
							}
							else
