	...
	implements FormPanel, ActionListener, TableModelListener, ListSelectionListener
{
		...
		BigDecimal qty)
	{
		...
		try {
			MInOutLine sLine = new MInOutLine (Env.getCtx(), M_InOutLine_ID, trx);
				...
				if (sLine.getM_Product_ID() != 0)
				{
					...
					else
					{
						...
						if (sLine.getProduct() != null && sLine.getProduct().isStocked())
							success = Storage.addQtys(Env.getCtx(), oLine.getM_Warehouse_ID(), 
								M_Locator_ID,	
								oLine.getM_Product_ID(), 
								oLine.getM_AttributeSetInstance_ID(),  
								null, null, qty.negate(), trx);
					}
				}
				else
					success = true;
