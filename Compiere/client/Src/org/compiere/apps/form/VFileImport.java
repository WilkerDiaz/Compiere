	...
	implements FormPanel, ActionListener
{
	...
	private void dynInit()
	{
		...
		String sql = "SELECT Name from AD_ImpFormat WHERE AD_CLIENT_ID IN (0,?)"; //Filtering data on tenant basis, add System import formats
		int AD_Client_ID = Env.getCtx().getAD_Client_ID();//Getting value from context
		try
		{
	...
	void cmd_process()
	{
		...
		for (row = 0; row < m_data.size(); row++) {
			int updated = m_format.updateDB(Env.getCtx(), m_data.get(row), null);
			if(updated == 0)
				imported++;
			else if (updated == 1)
				merged++;
		...
		ADialog.info(m_WindowNo, this, "FileImportR/I", row + " / " 
										+ Msg.getMsg(Env.getCtx(), "Inserted") + " : " + imported 
										+ Msg.getMsg(Env.getCtx(), " Merged") + " : " + merged );
		dispose();
	}	//	cmd_process
