...
implements DataStatusListener, ChangeListener, ActionListener, ASyncProcess
{
				...
				for (int tab = 0; tab < tabSize; tab++)
				{
					...
					if (tab == 0)
					{
						...
						if (m_mWorkbench.getWindowCount() == 1)
						{
							...
							if (query != null && query.getRecordCount() <= 1)
								goSingleRow = true;
							if(query==null && (gTab.isHighVolume() || (MRole.getDefault().isQueryRequire(getCount(gTab,query)))))
							{
								dispose();
								return false;
							}
					...
					if (gTab.isSortTab())
					{
						VSortTab st = new VSortTab(m_curWindowNo, gTab.getAD_Table_ID(),
								gTab.getAD_ColumnSortOrder_ID(), gTab.getAD_ColumnSortYesNo_ID(), gTab.isReadOnly());
	...
	public int getCount(GridTab mTab, Query query)
	{
		StringBuffer where = new StringBuffer();
		String wh1 = Evaluator.replaceVariables(mTab.getWhereExtended(), m_ctx, null);
		...
		int no = QueryUtil.getSQLValue(null, sql.toString());
		return no;
	}
	/**
	 *  Get Window Index
	...
	public void stateChanged (ChangeEvent e)
	{
		...
		else	//	Cur Tab Setting
		{
			...
			{
				if(m_curTab.getTableModel().hasChanged(m_curTab.getCurrentRow()))
				{
	...
	private void cmd_delete()
	{
		...
		if (!MRole.getDefault().isClientAccess(m_curTab.getAD_Client_ID(), true))
		{
			ADialog.error(m_curWindowNo, this, "CannotDelete");
			return;
		}
	...
	private void cmd_report ()
	{
		...
		if (queryColumn.length() != 0)
		{
			if (queryColumn.toUpperCase().endsWith("_ID"))
		...
		new AReport (m_curTab.getAD_Table_ID(), aReport.getButton(), query,m_curTab.getAD_Tab_ID());		
	}	//	cmd_report
	...
	private void cmd_zoomAcross()
	{
		...
		if (link.length() != 0)
		{
			if (link.toUpperCase().endsWith("_ID"))
	...
	private void actionButton (VButton vButton)
	{
		...
		//	Ensure it's saved
		if (record_ID == -1 && m_curTab.getKeyColumnName().toUpperCase().endsWith("_ID"))
		{
		...
		pi.setAD_Client_ID (m_ctx.getAD_Client_ID());
		pi.setIsBatch(batch);
		pi.setDateScheduledStart(dateScheduledStart);		
