...
//	private static Logger	s_log = Logger.getCLogger(OrderServlet.class);
		...
		throws ServletException, IOException
	{
		...
		{
			session.setAttribute(CheckOutServlet.ATTR_CHECKOUT, "Y");	//	indicate checkout
			url = "/login.jsp";
			done = true;
		}
		...
		// order confirmed, create order and forward to payment servlet
		String confirmed = WebUtil.getParameter (request, "orderConfirm");
		if (!done && confirmed != null) {
			MPayment p = (MPayment)session.getAttribute (PaymentServlet.ATTR_PAYMENT);
		...
		if (!done)
		{
			if (wb.getTotal().compareTo(Env.ZERO) > 0)
			{
				MPayment p = createPayment (session, ctx, wu, wb);
				if (p == null)
				{
...
//			}
	...
	/**************************************************************************
	 * 	Create temp Payment from Cart/Basket
	 * 	@param session session
	 * 	@param ctx context
	 * 	@param wu web user
	 * 	@param wb web basket
	 ...
	 */
	private MPayment createPayment(HttpSession session, Ctx ctx,
		WebUser wu, WebBasket wb)
	{
		//	See PaymentServlet.doGet
		MPayment p = new MPayment(ctx, 0, null);
		p.setAD_Org_ID(ctx.getAD_Org_ID());
		p.setIsSelfService(true);
		p.setAmount (wb.getCurrency_ID(), wb.getTotal()); //	for CC selection
		p.setIsOnline (true);
		//	Sales CC Trx
		p.setC_DocType_ID(true);
		p.setTrxType(X_C_Payment.TRXTYPE_Sales);
	...
	private boolean processOrder (HttpServletRequest request, MOrder order)
	{
		...
		boolean ok = DocumentEngine.processIt(order, DocAction);
		order.save();
		return ok;
	}	//	processOrder
