...
public class WebBasket
{
	...
	/**	Lines						*/
	private List<WebBasketLine>	m_lines = new ArrayList<WebBasketLine>();
	 ...
	 * 	@return lines
	 */
	public List<WebBasketLine> getLines()
	{
		return m_lines;
	}	//	getLines
	 ...
	 *	@return added line
	 */
	public WebBasketLine update (int prodId, BigDecimal qty)
	{
		WebBasketLine wbl = getLineFromProduct(prodId);
		if (wbl != null)
			wbl.setQuantity(qty);
		return wbl;
	}	//	update
	 ...
	 *	@return added line
	 */
	public WebBasketLine addOrUpdate(int prodId, String prodName, BigDecimal qty, BigDecimal price)
	{
		WebBasketLine wbl = getLineFromProduct(prodId);
		if (wbl != null)
			wbl.setQuantity(qty);
		else
			wbl = add(prodId, prodName, qty, price);
			
		return wbl;
	}	//	addOrUpdate

	public WebBasketLine deleteItem(int prodId)
	{
		WebBasketLine wbl = getLineFromProduct(prodId);
		if (wbl != null)
			delete(wbl.getLine());
	...
	public WebBasketLine getLineFromProduct(int prodId) {
		for (int i = 0; i < m_lines.size(); i++)
		{
			WebBasketLine wbl = m_lines.get(i);
			if (wbl.getM_Product_ID() == prodId)
				return wbl;
		}
		return null;
	}
