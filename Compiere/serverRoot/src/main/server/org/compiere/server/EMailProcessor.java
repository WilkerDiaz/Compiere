 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: EMailProcessor.java 7959 2009-08-08 14:46:15Z dzhao $
 */
public class EMailProcessor
{
	...
	private Store getStore() throws Exception
	{
		if (m_store != null)
			return m_store;
		if (getSession() == null)
			throw new CompiereStateException("No Session");
	...
	private int processInBox() throws Exception
	{
		...
		if (folder == null)
			throw new CompiereStateException("No default folder");
		//	Open Inbox
		Folder inbox = folder.getFolder("INBOX");
		if (!inbox.exists())
			throw new CompiereStateException("No Inbox");
		...
		if (!requestFolder.exists() && !requestFolder.create(Folder.HOLDS_MESSAGES))
			throw new CompiereStateException("Cannot create Request Folder");
		...
		if (!workflowFolder.exists() && !workflowFolder.create(Folder.HOLDS_MESSAGES))
			throw new CompiereStateException("Cannot create Workflow Folder");
		...
		if (!errorFolder.exists() && !errorFolder.create(Folder.HOLDS_MESSAGES))
			throw new CompiereStateException("Cannot create Error Folder");
