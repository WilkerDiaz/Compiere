 ...
 *  </pre>
 *  @author Jorg Janke
 *  @version  $Id: Doc_Production.java 8778 2010-05-19 19:00:10Z ragrawal $
 */
public class Doc_Production extends Doc
{
	...
	private DocLine[] loadLines(X_M_Production prod)
	{
		...
		try
		{
			pstmtPP = DB.prepareStatement(sqlPP, getTrx());
			...
			while (rsPP.next())
			{
				...
				try
				{
					pstmtPL = DB.prepareStatement(sqlPL, getTrx());
					pstmtPL.setInt(1, M_ProductionPlan_ID);
					rsPL = pstmtPL.executeQuery();
					while (rsPL.next())
					{
						X_M_ProductionLine line = new X_M_ProductionLine(getCtx(), rsPL, getTrx());
				...
				{
					DB.closeResultSet(rsPL);
					DB.closeStatement(pstmtPL);
				}
			}
		}
		...
		{
			DB.closeResultSet(rsPP);
			DB.closeStatement(pstmtPP);
		}
