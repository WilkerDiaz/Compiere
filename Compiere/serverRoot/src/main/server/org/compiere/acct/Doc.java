 ...
 *  @author Jorg Janke
 *  @version  $Id: Doc.java 8936 2010-06-11 10:14:35Z ragrawal $
 */
public abstract class Doc implements AccountingInterface
{
	...
	public static Doc get (MAcctSchema[] ass, int AD_Table_ID, int Record_ID, Trx trx)
	{
		...
		//Get the Doc Type
		int C_DocBaseType_ID = 0;
		int index = MTable.get(ctx, AD_Table_ID).get_ColumnIndex("C_DocType_ID");
		if(index>0) 
		{
			String tName = MTable.getTableName(ctx, AD_Table_ID);
			String sql1 = " SELECT b.C_DocBaseType_ID "
			       + " FROM C_DocType d, C_DocBaseType b, "
			       + tName + " x "
			       + " WHERE d.C_DocType_ID = x.C_DocType_ID "
			       + " AND b.DocBaseType = d.DocBaseType "
			       + " AND x."+tName+"_ID = ?";
		C_DocBaseType_ID = QueryUtil.getSQLValue(trx, sql1, Record_ID);
		}

		MDocBaseType dbt = MDocBaseType.getForTable(ctx, AD_Table_ID,C_DocBaseType_ID);
		if (dbt == null)
		{
	...
	public static Doc get (MAcctSchema[] ass, int AD_Table_ID, ResultSet rs, Trx trx)
	{
		...
		if(C_DocType_ID >0)
		{
			String sql = " SELECT b.C_DocBaseType_ID "
				       + " FROM C_DocBaseType b, C_DocType d "
				       + " WHERE b.DocBaseType = d.DocBaseType "
				       + " AND d.C_DocType_ID = ? ";
			C_DocBaseType_ID = QueryUtil.getSQLValue(trx, sql, C_DocType_ID);
		}
	...
	public Doc (MAcctSchema[] ass, Class<?> clazz, ResultSet rs, String defaultDocumentType, Trx trx)
	{
		...
		m_trx = trx;
		if (m_trx == null)
			m_trx = Trx.get("Post" + m_DocumentType + p_po.get_ID());
		p_po.set_Trx(m_trx);
	...
	/** Work Center                 */
	protected int               m_workcenter_id = -1;
	/** Cost Element ID for Work Center Cost */
	protected int               m_costelement_id = -1;
	/** if the document is reposted */
	protected boolean           m_repost = false;
	...
	public final String post (boolean force, boolean repost)
	{
		...
		if (DB.executeUpdate(getTrx(), sql.toString(), new Object[]{get_ID()}) == 1)	//	outside p_trx
				...
				if (m_ass[i].getAD_OrgOnly_ID() != 0)
				{
					if (m_ass[i].getOnlyOrgs() == null)
						m_ass[i].setOnlyOrgs(MReportTree.getChildIDs(getCtx(),
							...
							m_ass[i].getAD_OrgOnly_ID(), false));
		...
		if (!p_Status.equals(STATUS_Posted))
		{
			...
			MNote note = new MNote (getCtx(), AD_MessageValue, AD_User_ID,
				getAD_Client_ID(), getAD_Org_ID(), getTrx());
	...
	private int deleteAcct()
	{
		...
		int no = DB.executeUpdate(getTrx(), sql, new Object[]{get_Table_ID(), p_po.get_ID()});
		if (no != 0)
			log.info("#=" + no);
		return no;
	}	//	deleteAcct
	...
	private void unlock()
	{
		...
		sql.append(get_TableName()).append( " SET Processing='N' WHERE ")
			.append(get_TableName()).append("_ID=?");
		DB.executeUpdate((Trx) null, sql.toString(), new Object[]{p_po.get_ID()});		//	outside p_trx
	}   //  unlock
	...
	protected void setDocumentType (String DocumentType)
	{
		...
		if (m_DocumentType == null)
			throw new CompiereStateException("Document Type not found");
	}	//	setDocumentType
	...
	public static final int ACCTTYPE_WOMaterialAcct = 1773;
	/** Work Order Material Overhead Account */
	public static final int ACCTTYPE_WOMaterialOverhdAcct = 1774;
	/** Work Order Resource Account */
	public static final int ACCTTYPE_WOResourceAcct = 1775;
	/** Work Center Overhead Account */
	public static final int ACCTTYPE_WCOverhdAcct = 1777;
	/** Work Order Material Variance Account */
	public static final int ACCTTYPE_WOMaterialVarianceAcct = 7672;
	/** Work Order Material Overhead Variance Account */
	public static final int ACCTTYPE_WOMaterialOverhdVarianceAcct = 7673;
	/** Work Order Resource Variance Account */
	public static final int ACCTTYPE_WOResoureVarianceAcct = 7674;
	/** Work Order Overhead Variance Account */
	public static final int ACCTTYPE_WOOverhdVarianceAcct = 7675;
	/** Work Order Scrap Account */
	public static final int ACCTTYPE_WOScrapAcct = 7677;
	/** Overhead Absorption Account */
	public static final int ACCTTYPE_OverhdAbsorptionAcct = 7678;  //TODO Check the account number
	...
	public int getValidCombination_ID (int AcctType, MAcctSchema as)
	{
		...
		if (AcctType == ACCTTYPE_Charge)	//	see getChargeAccount in DocLine
		{
			...
			if (cmp == 0)
				...
				sql = " SELECT COALESCE(a.CH_Expense_Acct,b.CH_Expense_Acct) "
					+ " FROM C_Charge_Acct a "
					+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
					+ " WHERE a.C_Charge_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			else
				sql = " SELECT COALESCE(a.CH_Revenue_Acct, b.CH_Revenue_Acct) "
					+ " FROM C_Charge_Acct a "
					+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
					+ " WHERE a.C_Charge_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
		...
		{
			sql = " SELECT COALESCE(a.V_Liability_Acct, b.V_Liability_Acct) "
		...
		{
			sql = " SELECT COALESCE(a.V_Liability_Services_Acct, b.V_Liability_Services_Acct) "
				+ " FROM C_BP_Vendor_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.C_BPartner_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_BPartner_ID();
		...
		{
			sql = " SELECT COALESCE(a.C_Receivable_Acct,b.C_Receivable_Acct) "
				+ " FROM C_BP_Customer_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.C_BPartner_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_BPartner_ID();
		}
		...
		{
			sql = " SELECT COALESCE(a.C_Receivable_Services_Acct,b.C_Receivable_Services_Acct) "
				+ " FROM C_BP_Customer_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.C_BPartner_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_BPartner_ID();
		...
		{
			sql = " SELECT COALESCE(a.V_Prepayment_Acct,b.V_Prepayment_Acct) "
				+ " FROM C_BP_Vendor_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.C_BPartner_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_BPartner_ID();
		...
		{
			sql = " SELECT COALESCE(a.C_Prepayment_Acct,b.C_Prepayment_Acct) "
				+ " FROM C_BP_Customer_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.C_BPartner_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_BPartner_ID();
		...
		{
			sql = " SELECT COALESCE(a.B_UnallocatedCash_Acct, b.B_UnallocatedCash_Acct) "
				+ " FROM C_BankAccount_Acct a " 
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.C_BankAccount_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_BankAccount_ID();
		}
		else if (AcctType == ACCTTYPE_BankInTransit)
		{
			sql = " SELECT COALESCE(a.B_InTransit_Acct, b.B_InTransit_Acct) "
				+ " FROM C_BankAccount_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.C_BankAccount_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_BankAccount_ID();
		...
		{
			sql = " SELECT COALESCE(a.B_PaymentSelect_Acct, b.B_PaymentSelect_Acct) "
				+ " FROM C_BankAccount_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.C_BankAccount_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_BankAccount_ID();
		...
		{
			sql = " SELECT COALESCE(a.PayDiscount_Exp_Acct, b.PayDiscount_Exp_Acct) "
				+ " FROM C_BP_Group_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " ,C_BPartner bp "
				+ "WHERE a.C_BP_Group_ID=bp.C_BP_Group_ID AND bp.C_BPartner_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_BPartner_ID();
		}
		else if (AcctType == ACCTTYPE_DiscountRev)
		{
			sql = " SELECT COALESCE(a.PayDiscount_Rev_Acct,b.PayDiscount_Rev_Acct) "
				+ " FROM C_BP_Group_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " ,C_BPartner bp "
				+ " WHERE a.C_BP_Group_ID=bp.C_BP_Group_ID AND bp.C_BPartner_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
		...
		{
			sql = " SELECT COALESCE(a.WriteOff_Acct, b.WriteOff_Acct) "
				+ " FROM C_BP_Group_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " ,C_BPartner bp "
				+ " WHERE a.C_BP_Group_ID=bp.C_BP_Group_ID AND bp.C_BPartner_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
		...
		{
			sql = " SELECT COALESCE(a.B_Asset_Acct,b.B_Asset_Acct) "
				+ " FROM C_BankAccount_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.C_BankAccount_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_BankAccount_ID();
		...
		{
			sql = " SELECT COALESCE(a.B_InterestRev_Acct, b.B_InterestRev_Acct) "
				+ " FROM C_BankAccount_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.C_BankAccount_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_BankAccount_ID();
		...
		{
			sql = " SELECT COALESCE(a.B_InterestExp_Acct, b.B_InterestExp_Acct) "
				+ " FROM C_BankAccount_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.C_BankAccount_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_BankAccount_ID();
		...
		{
			sql = " SELECT COALESCE(a.CB_Asset_Acct, b.CB_Asset_Acct) "
				+ " FROM C_CashBook_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.C_CashBook_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_CashBook_ID();
		}
		else if (AcctType == ACCTTYPE_CashTransfer)
		{
			sql = " SELECT COALESCE(a.CB_CashTransfer_Acct, b.CB_CashTransfer_Acct) "
				+ " FROM C_CashBook_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.C_CashBook_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_CashBook_ID();
		...
		{
			sql = " SELECT COALESCE(a.CB_Expense_Acct, b.CB_Expense_Acct) "
				+ " FROM C_CashBook_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.C_CashBook_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_CashBook_ID();
		...
		{
			sql = " SELECT COALESCE(a.CB_Receipt_Acct, b.CB_Receipt_Acct) "
				+ " FROM C_CashBook_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.C_CashBook_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_CashBook_ID();
		...
		{
			sql = " SELECT COALESCE(a.CB_Differences_Acct, b.CB_Differences_Acct) "
				+ " FROM C_CashBook_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.C_CashBook_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_CashBook_ID();
		...
		{
			sql = " SELECT COALESCE(a.W_Differences_Acct, b.W_Differences_Acct) "
				+ " FROM M_Warehouse_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.M_Warehouse_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getM_Warehouse_ID();
		}
		else if (AcctType == ACCTTYPE_NotInvoicedReceipts)
		{
			sql = " SELECT COALESCE(a.NotInvoicedReceipts_Acct, b.NotInvoicedReceipts_Acct) "
				+ " FROM C_BP_Group_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ ", C_BPartner bp "
				+ " WHERE a.C_BP_Group_ID=bp.C_BP_Group_ID AND bp.C_BPartner_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
		...
		{
			sql = " SELECT COALESCE(a.PJ_Asset_Acct, b.PJ_Asset_Acct) "
				+ " FROM C_Project_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.C_Project_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_Project_ID();
		}
		else if (AcctType == ACCTTYPE_ProjectWIP)
		{
			sql = " SELECT COALESCE(a.PJ_WIP_Acct, b.PJ_WIP_Acct) "
				+ " FROM C_Project_Acct a "
				+ " INNER JOIN C_AcctSchema_Default b ON (a.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE a.C_Project_ID=? AND a.C_AcctSchema_ID=? AND a.IsActive = 'Y' ";
			para_1 = getC_Project_ID();
		...
		else if (AcctType == ACCTTYPE_WOMaterialAcct)
		{
			sql = " SELECT COALESCE(woclass.WO_Material_Acct, b.WO_Material_Acct) "
				+ " FROM M_WorkOrderClass_Acct woclass "
				+ " INNER JOIN C_AcctSchema_Default b ON (woclass.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ ", M_WorkOrder wo "
				+ "WHERE wo.M_WorkOrder_ID = ? AND woclass.M_WorkOrderClass_ID = wo.M_WorkOrderClass_ID AND woclass.C_AcctSchema_ID=?  AND woclass.IsActive = 'Y' ";
		...
		else if (AcctType == ACCTTYPE_WCOverhdAcct)
		{
			sql = " SELECT COALESCE(wc.WC_Overhead_Acct,b.WC_Overhead_Acct) "
				+ " FROM M_WorkCenter_Acct wc"
				+ " INNER JOIN C_AcctSchema_Default b ON (wc.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE wc.M_WorkCenter_ID = ? AND wc.C_AcctSchema_ID=?  AND wc.IsActive = 'Y' ";
			para_1 = m_workcenter_id;
		}
		...
		else if (AcctType == ACCTTYPE_OverhdAbsorptionAcct)
		{
			sql = " SELECT COALESCE(wcc.Overhead_Absorption_Acct, b.Overhead_Absorption_Acct) "
				+ " FROM M_WorkCenterCost wcc "
				+ " INNER JOIN C_AcctSchema_Default b ON (wcc.C_AcctSchema_ID = b.C_AcctSchema_ID)"
				+ " WHERE wcc.M_WorkCenter_ID = ? " 
				+ " AND wcc.C_AcctSchema_ID=? "
				+ " AND wcc.M_CostElement_ID = ? " 
				+ " AND wcc.M_CostType_ID = ? ";
			para_1 = m_workcenter_id;
		}
		else
		{
	...
	private final boolean save(Trx trx)
	{
		...
		StringBuffer sql = new StringBuffer("UPDATE ");
		sql.append(get_TableName()).append(" SET Posted=?")
			.append(" ,Processing='N' ")
			.append("WHERE ")
			.append(get_TableName()).append("_ID=?");
	...
	public Timestamp getDateAcct()
	{
		...
		throw new CompiereStateException("No DateAcct");
	}	//	getDateAcct
	...
	public Timestamp getDateDoc()
	{
		...
		throw new CompiereStateException("No DateDoc");
	}	//	getDateDoc
	...
	public boolean isPosted()
	{
		...
		throw new CompiereStateException("No Posted");
	}	//	isPosted
