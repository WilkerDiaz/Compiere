 ...
 *
 *  @author 	Jorg Janke
 *  @version 	$Id: DocLine.java 8780 2010-05-19 23:08:57Z nnayak $
 */
...
{	
	...
	public int getOrder_Org_ID()
	{
		...
		if (C_OrderLine_ID != 0)
		{
			...
			int AD_Org_ID = QueryUtil.getSQLValue(getTrx(), sql, C_OrderLine_ID);
			if (AD_Org_ID > 0)
				return AD_Org_ID;
		}
		return getAD_Org_ID();
	...
	public int getC_SalesRegion_ID()
	{
		if (m_C_SalesRegion_ID == -1)	//	never tried
		{
			...
			//	&& m_acctSchema.isAcctSchemaElement(MAcctSchemaElement.ELEMENTTYPE_SalesRegion))
			{
				...
				m_C_SalesRegion_ID = QueryUtil.getSQLValue (getTrx(),
	...
	{
		int index = p_po.get_ColumnIndex("BasisType");
		if (index != -1)
			return (String)p_po.get_Value(index);
		return null;
	}	//	getBasisType
	...
	{
		int index = p_po.get_ColumnIndex("M_WorkOrderOperation_ID");
		if (index != -1)
		{
			Integer ii = (Integer)p_po.get_Value(index);
			if (ii !=null)
