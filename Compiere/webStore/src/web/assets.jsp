	...
	<div id="content">
	  ...
	  <c:if test='${not webUser.EMailVerified}'>
        <form action="emailServlet" method="post" enctype="application/x-www-form-urlencoded"
			...
			<p><b>To access your downloads:</b></p>
      ...
      <table class="contentTable">
        ...
        <tr> 
          <td class="<c:out value='${rowClass}' /> left"><c:out value='${asset.name}'/></td>
          <td class="<c:out value='${rowClass}' /> left"><c:out value='${asset.description}'/>&nbsp;</td>
          <td class="<c:out value='${rowClass}' /> left"><fmt:formatDate value='${asset.guaranteeDate}'/></td>
          ...
          <td class="<c:out value='${rowClass}' /> left">
		    <c:if test='${asset.downloadable}'>
		    	...
		    	<a href="<c:out value='${ctx.contextPlain}/'/>assetServlet/<c:out value='${addlDL}'/>.zip?Asset_ID=<c:out value='${asset.a_Asset_ID}'/>&PD=<c:out value='${addlDL}'/>" target="_blank">
		  	  	<img src="res/assetDownload.gif" alt="Download <c:out value='${addlDL}'/>" width="24" height="24" border="0"></a><br>
			  </c:forEach>
			</c:if>
