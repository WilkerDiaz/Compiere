 ...
 * 	Cost Element Model
 *  @author Jorg Janke
 *  @version $Id: MCostElement.java 8780 2010-05-19 23:08:57Z nnayak $
 */
public class MCostElement extends X_M_CostElement
{
    /** Logger for class MCostElement */
    private static final org.compiere.util.CLogger log = org.compiere.util.CLogger.getCLogger(MCostElement.class);
	/**
	 * 
	 */
	...
	public static MCostElement getMaterialCostElement (PO po, String CostingMethod)
	{
		...
		String name = MRefList.getListName(po.getCtx(), X_Ref_C_AcctSchema_Costing_Method.AD_Reference_ID, CostingMethod);
	...
	protected boolean beforeSave (boolean newRecord)
	{
			...
			&& (newRecord || is_ValueChanged("CostingMethod")))
		{
			...
			int id = QueryUtil.getSQLValue(get_Trx(), sql, getAD_Client_ID(), getCostingMethod());
			if (id > 0 && id != get_ID())
			{
		...
		if (getAD_Org_ID() != 0)
			setAD_Org_ID(0);
		if(isMfgMaterialCost())
		{
			int ce = MCostElement.getMfgCostElement(getAD_Client_ID());
			if(ce !=0)
			{
				log.saveError("AlreadyExists", "Manufacturing Material Cost Element ");
				return false;
			}
		}
		return true;
	...
	{
		int ce = 0;
		String sql = " SELECT M_CostElement_ID FROM M_CostElement WHERE AD_Client_ID = ? AND IsMfgMaterialCost = 'Y'";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try
		{
