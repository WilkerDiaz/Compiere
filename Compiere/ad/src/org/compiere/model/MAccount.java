 ...
 *
 *  @author		Jorg Janke
 *  @version 	$Id: MAccount.java 9155 2010-08-03 08:54:53Z ragrawal $
 */
public class MAccount extends X_C_ValidCombination
{
    /** Logger for class MAccount */
    private static final org.compiere.util.CLogger log = org.compiere.util.CLogger.getCLogger(MAccount.class);
	/**
	 * 
	 */
	 ...
	 *	@param trx transaction
	 */
	public static void updateValueDescription (Ctx ctx, String where, Trx trx,Object[] params)
	{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (newRecord || is_ValueChanged("Alias"))
		{
			...
			int ii = QueryUtil.getSQLValue(null, sql, getAlias());
			if (ii != 0)
			{
	...
	@UICallout public void setC_AcctSchema_ID (String oldC_AcctSchema_ID,
			String newC_AcctSchema_ID, int windowNo) throws Exception
	{
		if ((newC_AcctSchema_ID == null) || (newC_AcctSchema_ID.length() == 0))
			return;
		int C_AcctSchema_ID = Integer.parseInt(newC_AcctSchema_ID);
		if (C_AcctSchema_ID == 0)
			return;
		
		p_changeVO.addChangedValue("Element_AY","N");
		p_changeVO.addChangedValue("Element_BP","N");
		p_changeVO.addChangedValue("Element_LF","N");
		p_changeVO.addChangedValue("Element_LT","N");
		p_changeVO.addChangedValue("Element_MC","N");
		p_changeVO.addChangedValue("Element_OT","N");
		p_changeVO.addChangedValue("Element_PJ","N");
		p_changeVO.addChangedValue("Element_PR","N");
		p_changeVO.addChangedValue("Element_SA","N");
		p_changeVO.addChangedValue("Element_SR","N");
		p_changeVO.addChangedValue("Element_U1","N");
		p_changeVO.addChangedValue("Element_U2","N");
		p_changeVO.addChangedValue("Element_X1","N");
		p_changeVO.addChangedValue("Element_X2","N");
		...
		Object result[][]= QueryUtil.executeQuery(get_Trx(), sql, C_AcctSchema_ID);
		for (Object r[] : result)
			p_changeVO.addChangedValue("Element_"+(String)r[0], "Y");
