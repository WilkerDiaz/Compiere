 ...
 *
 * 	@author 	Jorg Janke
 * 	@version 	$Id: MPrintColor.java 8755 2010-05-12 18:30:14Z nnayak $
 */
public class MPrintColor extends X_AD_PrintColor
{
    /** Logger for class MPrintColor */
    private static final org.compiere.util.CLogger log = org.compiere.util.CLogger.getCLogger(MPrintColor.class);
	/**
	 * 
	 */
...
**/
	...
	/**
	 * 	Get XML for all Print Colors
	 * 	@param ctx context
	 * 	@param rw read write
	 * 	@return print Colors
	 */
	public static StringBuffer getPrintColorsXML( Ctx ctx, boolean rw )
	{
		StringBuffer colors_xml = new StringBuffer("<AD_PrintColors>");

		MRole role = MRole.get(ctx, ctx.getAD_Role_ID(), ctx.getAD_User_ID(), false);
		
		String pc_sql = role.addAccessSQL("SELECT PC.AD_PrintColor_ID, PC.Name FROM AD_PrintColor PC " +
						"ORDER BY Name ASC",
						"AD_PrintColor", MRole.SQL_NOTQUALIFIED, rw);
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try
		{
			pstmt = DB.prepareStatement(pc_sql, (Trx) null);
			rs = pstmt.executeQuery();
			while( rs.next() )
			{
				colors_xml.append("<AD_PrintColor>");

				colors_xml.append("<AD_PrintColor_ID>");
				colors_xml.append(rs.getInt(1));
				colors_xml.append("</AD_PrintColor_ID>");

				colors_xml.append("<Name>");
				colors_xml.append(rs.getString(2));
				colors_xml.append("</Name>");	

				MPrintColor pc = MPrintColor.get(ctx, rs.getInt(1));
				colors_xml.append("<RGB>");
				colors_xml.append("0x" + pc.getRRGGBB());
				colors_xml.append("</RGB>");	

				colors_xml.append("</AD_PrintColor>");
			}
			colors_xml.append("</AD_PrintColors>");
		}
		catch(SQLException e) {
			log.log(Level.SEVERE, pc_sql, e);
		}
		finally {
