...
public class TableElement extends PrintElement
{
	...
	protected boolean calculateSize()
	{
		...
		for (int dataCol = 0; dataCol < cols; dataCol++)
		{
			...
			for (int row = 0; row < rows; row++)
			{
				...
				Color color = getColor(row,dataCol);
				//	Print below existing column = (col != dataCol)
				addPrintLines(row, col, dataItem,font,color);
		...
		final int firstRow, final int nextPageRow, boolean isView)
	{
		...
		for (int row = firstRow; row < nextPageRow; row++)
		{
			...
			//	actual data
			Object[] printItems = (Object[]) getPrintItems(row, col);		
			float penY = curY;
			for (Object printItem : printItems){
				Object element = ((PrintDataHelper)printItem).getData();			
				if (element == null)
					;
				...
				else
				{
					...
					if (str.length() > 0)
					{
						...
						{
							if(lines[lineNo] == null || lines[lineNo].isEmpty())
								continue;
							
							aString = new AttributedString(lines[lineNo]);
							aString.addAttribute(TextAttribute.FONT, ((PrintDataHelper)printItem).getFont());
									...
									element instanceof NamePair)	//	ID
							{
								...
								// add try catch block as string with newline characters raise an IllegalArgumentException
								try{
									aString.addAttribute(TextAttribute.UNDERLINE, TextAttribute.UNDERLINE_LOW_ONE_PIXEL, 0, str.length());
								}
								catch (IllegalArgumentException e){
									log.log(Level.SEVERE,"Error while underlining string "+str);
								}
							}
							else
								aString.addAttribute(TextAttribute.FOREGROUND, ((PrintDataHelper)printItem).getColor());
							//
							iter = aString.getIterator();
							boolean fastDraw = LayoutEngine.s_FASTDRAW;
	 ...
	 * 	@param data data
	 */
	private void addPrintLines (int row, int col, Object data,Font font,Color color)
	{
