...
{
    /** Logger for class MDocTypeCounter */
    private static final org.compiere.util.CLogger log = org.compiere.util.CLogger.getCLogger(MDocTypeCounter.class);
	/**
	 * 
	 */
	...
	public static int getCounterDocType_ID (Ctx ctx, int C_DocType_ID)
	{
		...
		if (dtCounter != null)
		{
			if (!dtCounter.isCreateCounter() || !dtCounter.isValid() || !dtCounter.isActive())
				return -1;
	...
	public static MDocTypeCounter getCounterDocType (Ctx ctx, int C_DocType_ID)
	{
		...
		try
		{
			...
			while (rs.next () && retValue == null)
			{
				...
				if (!retValue.isCreateCounter() || !retValue.isValid()|| !retValue.isActive())
				{
					temp = retValue; 
					retValue = null;
				}
