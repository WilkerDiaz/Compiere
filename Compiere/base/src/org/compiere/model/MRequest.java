...
{
    /** Logger for class MRequest */
    private static final org.compiere.util.CLogger log = org.compiere.util.CLogger.getCLogger(MRequest.class);
	/**
	 *
	 */
	private static final long serialVersionUID = 1L;
	private int I_Request_ID=0;
	...
	public MRequest (X_I_Request imp)
	{
		...
		I_Request_ID=imp.getI_Request_ID();
		
		if (imp.getDueType()==null)
			setDueType (DUETYPE_Due);
		
		if (imp.getConfidentialType()==null)
			setConfidentialType (CONFIDENTIALTYPE_PublicInformation);	// A
		
		if (imp.getConfidentialTypeEntry()==null)
			setConfidentialTypeEntry (CONFIDENTIALTYPEENTRY_PublicInformation);	// A
		
		if (imp.getPriorityUser()==null)
			setPriorityUser (PRIORITY_Low);
	...
	{
		return MRefList.getListName(getCtx(), X_Ref_R_Request_Due_Type.AD_Reference_ID, getDueType());
	}	//	getDueTypeText
	...
	{
		return MRefList.getListName(getCtx(), X_Ref__PriorityRule.AD_Reference_ID, getPriority());
	}	//	getPriorityText
	...
	{
		return MRefList.getListName(getCtx(), X_Ref__PriorityRule.AD_Reference_ID, getPriorityUser());
	}	//	getPriorityUserText
	...
	{
		return MRefList.getListName(getCtx(), X_Ref_R_Request_Confidential.AD_Reference_ID, getConfidentialType());
	}	//	getConfidentialText
	...
	protected boolean beforeDelete() {
		MRequestUpdate[] updates = getUpdates(null);
		if(updates.length >0)
		{
			log.saveError("Child Record Exists", "Cannot delete Request as child record exist for this request");
			return false;
		}
		MRequestAction[] actions = MRequestAction.getActions(this);
		if(actions.length >0)
		{
			log.saveError("Child Record Exists", "Cannot delete Request as child record exist for this request");
			return false;
