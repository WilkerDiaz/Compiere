...
implements Serializable
{
	...
	public final void setValueAt (Object value, int row, int col, boolean force)
	{
		...
		int valueCompare = 0;
		if(oldValue != null && value != null && oldValue instanceof BigDecimal && value instanceof BigDecimal)
		{
			BigDecimal decimalOldValue = (BigDecimal)oldValue;
			BigDecimal decimalValue = (BigDecimal)value;
			valueCompare = decimalOldValue.compareTo(decimalValue);
		}
		...
		if (!force && (
				...
				||	oldValue != null && value != null && 
				((oldValue instanceof BigDecimal && value instanceof BigDecimal && valueCompare == 0) 
						|| !(oldValue instanceof BigDecimal && value instanceof BigDecimal ) && oldValue.toString().equals(value.toString()))
	...
	private char dataSavePO (int Record_ID) throws Exception
	{
		...
		Trx trx = Trx.get("Data Save PO");
		
		if (table.isSingleKey() || Record_ID == 0)
			po = table.getPO(m_ctx, Record_ID, trx);
		else	//	Multi - Key
			po = table.getPO(m_ctx, getWhereClause(rowData), trx);
		...
		StringBuffer refreshSQL = new StringBuffer(m_SQL_Select)
		.append(" WHERE ").append(whereClause);
		PreparedStatement pstmt = DB.prepareStatement(refreshSQL.toString(), trx);
		ResultSet rs = null;
		try
		{
			rs = pstmt.executeQuery();
...
//		m_rowChanged = -1;  //  only changed in setValueAt
		...
		m_rowChanged = m_newRow;  //  force save checking on new record
		...
		try
		{
			...
			for (int j = 0; j < size; j++)
			{
				...
				if (displayType == DisplayTypeConstants.Integer
						...
						&& (columnName.toUpperCase().endsWith("_ID") || columnName.endsWith("_Acct"))
	...
	class Loader extends Thread implements Serializable
	{
		...
		protected int open (int maxRows)
		{
			...
			try
			{
				pstmt = DB.prepareStatement(m_SQL_Count, (Trx) null);
		...
		void close()
		{
			...
			{
				DB.closeStatement(m_pstmt);
				DB.closeResultSet(m_rs);
			}
			m_rs = null;
			m_pstmt = null;
		}	//	close
