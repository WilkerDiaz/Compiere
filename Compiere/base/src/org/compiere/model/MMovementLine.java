...
{
    /** Logger for class MMovementLine */
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (getLine() == 0)
		{
			...
			int ii = QueryUtil.getSQLValue (get_Trx(), sql, getM_Movement_ID());
			setLine (ii);
		}
		...
		if (getMovementQty().signum() == 0) 
			// Throw a warning only as quantity can be set to Zero when voiding the document
			log.saveWarning("FillMandatory", Msg.getElement(getCtx(), "MovementQty"));
