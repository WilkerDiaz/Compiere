...
public class ExpenseSOrder extends SvrProcess
{
	...
	private void processLine (MTimeExpense te, MTimeExpenseLine tel, MBPartner bp)
	{
		if (m_order == null)
		{
			...
			if (!m_order.save())
			{
				throw new CompiereStateException("Cannot save Order");
			}
		}
		else
		{
		...
		if (!ol.save())
		{
			throw new CompiereStateException("Cannot save Order Line");
		}
	...
	private void completeOrder ()
	{
		...
		DocumentEngine.processIt(m_order, DocActionConstants.ACTION_Prepare);
		if (!m_order.save())
			throw new CompiereStateException("Cannot save Order");
		m_noOrders++;
