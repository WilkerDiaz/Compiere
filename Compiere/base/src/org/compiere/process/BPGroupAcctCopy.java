...
public class BPGroupAcctCopy extends SvrProcess
{
	...
	protected String doIt () throws Exception
	{
		...
		sql = " UPDATE C_BP_Customer_Acct ca "
			+ " SET C_Receivable_Acct= (SELECT C_Receivable_Acct "
			                       + " FROM C_BP_Group_Acct "
			                       + " WHERE C_BP_Group_ID= ? "
			                       + " AND C_AcctSchema_ID= ? ), "
		    + " C_Receivable_Services_Acct = (SELECT C_Receivable_Services_Acct "
				                          + " FROM C_BP_Group_Acct"
				                          + " WHERE C_BP_Group_ID= ? "
				                          + " AND C_AcctSchema_ID= ? ), "
			+ " C_PrePayment_Acct = (SELECT C_PrePayment_Acct "
					             + " FROM C_BP_Group_Acct"
					             + " WHERE C_BP_Group_ID= ? "
					             + " AND C_AcctSchema_ID= ? ), "
			...
			+ " AND EXISTS (SELECT 1 "
				        ...
				        + " AND p.C_BP_Group_ID= ? )";
		Object[] params = new Object[]{p_C_BP_Group_ID,p_C_AcctSchema_ID,
				                       p_C_BP_Group_ID,p_C_AcctSchema_ID,
				                       p_C_BP_Group_ID,p_C_AcctSchema_ID,
				                       p_C_AcctSchema_ID,p_C_BP_Group_ID};
		...
		params = new Object[]{p_C_AcctSchema_ID,p_C_BP_Group_ID};
		created = DB.executeUpdate(get_TrxName(), sql,params);
		...
		sql = " UPDATE C_BP_Vendor_Acct va "
			+ " SET V_Liability_Acct = (SELECT V_Liability_Acct "
			                        + " FROM C_BP_Group_Acct "
			                        + " WHERE C_BP_Group_ID= ? "
			                        + " AND C_AcctSchema_ID= ? ), "
			+ " V_Liability_Services_Acct = (SELECT V_Liability_Services_Acct "
				                         + " FROM C_BP_Group_Acct "
				                         + " WHERE C_BP_Group_ID= ? "
				                         + " AND C_AcctSchema_ID= ? ), "
			+ " V_PrePayment_Acct = (SELECT V_PrePayment_Acct "
 					             + " FROM C_BP_Group_Acct "
					             + " WHERE C_BP_Group_ID= ? "
					             + " AND C_AcctSchema_ID= ? ), "
			+ " Updated=SysDate, UpdatedBy=0 "
			...
			+ " AND EXISTS (SELECT 1 " 
				        ...
				        + " AND p.C_BP_Group_ID= ? )";
		params = new Object[]{p_C_BP_Group_ID,p_C_AcctSchema_ID,
				              p_C_BP_Group_ID,p_C_AcctSchema_ID,
				              p_C_BP_Group_ID,p_C_AcctSchema_ID,
				              p_C_AcctSchema_ID,p_C_BP_Group_ID};
		updated = DB.executeUpdate(get_TrxName(), sql,params);
