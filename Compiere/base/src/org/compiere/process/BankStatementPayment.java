...
public class BankStatementPayment extends SvrProcess
{
	...
	private String createPayment (X_I_BankStatement ibs) throws Exception
	{
		...
		ibs.setC_BPartner_ID(payment.getC_BPartner_ID());
		if (payment.getC_Invoice_ID() != 0)
			ibs.setC_Invoice_ID(payment.getC_Invoice_ID());
		...
		BigDecimal interest = ibs.getInterestAmt();
		if (interest == null)
			interest = Env.ZERO;
		ibs.setChargeAmt(interest.negate());
		
		ibs.setDescription(payment.getDescription());
		...
		String Description, int AD_Org_ID)
	{
		...
		if (C_Invoice_ID == 0
			...
			throw new CompiereStateException ("@PayAmt@ = 0");
		if (PayAmt == null)
			PayAmt = Env.ZERO;
		//
