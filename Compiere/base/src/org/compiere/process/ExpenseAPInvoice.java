...
public class ExpenseAPInvoice extends SvrProcess
{
		...
		try
		{
			...
			while (rs.next())				//	********* Expense Line Loop
			{
				...
				if (te.getC_BPartner_ID() != old_BPartner_ID)
				{
					...
					if (!invoice.save())
						throw new CompiereStateException("Cannot save Invoice");
				...
				//	il.setPrice();	//	not really a list/limit price for reimbursements
					...
					il.setTax();
					if (!il.save())
						throw new CompiereStateException("Cannot save Invoice Line");
					//	Update TEL
	...
	private void completeInvoice (MInvoice invoice)
	{
		...
		DocumentEngine.processIt(invoice, DocActionConstants.ACTION_Prepare);
		if (!invoice.save())
			throw new CompiereStateException("Cannot save Invoice");
		//
		m_noInvoices++;
