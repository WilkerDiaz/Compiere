...
public class ChangeLogProcess extends SvrProcess
{
		...
		try
		{
			...
			while (rs.next ())
			{
				try{
					...
					System.out.println("QUERYYY::"+m_sqlUpdate+m_sqlUpdateWhere);
					
					System.out.println("m_sqlInsert");
	...
	private String getSQLValue (String value)
	{
		...
		if (FieldType.isNumeric (m_column.getAD_Reference_ID())
			|| (	FieldType.isID (m_column.getAD_Reference_ID())
				&&	!m_column.getColumnName().equalsIgnoreCase("EntityType")))
			return value;
		if (DisplayTypeConstants.YesNo == m_column.getAD_Reference_ID() 
				|| DisplayTypeConstants.Button == m_column.getAD_Reference_ID()) 
		{
	...
	private boolean executeStatement()
	{
		...
		if (m_isInsert && m_numberColumns > 2)
		{
			...
			//
			no = DB.executeUpdate(get_TrxName(), m_sqlInsert.toString());
			if (no == -1)
			{
		...
		else	//	Update SQL
		{
			...
			//
			no = DB.executeUpdate(get_TrxName(), m_sqlUpdate.toString());
			if (no == -1)
			{
	...
	private String setCustomization()
	{
		...
		int resetNo = DB.executeUpdate(get_TrxName(), sql);
		...
		try
		{
			...
			while (rs.next ())
			{
				...
				int no = DB.executeUpdate(get_TrxName(), update.toString(),table.getAD_Table_ID());
