	...
	protected String doIt() throws java.lang.Exception
	{
		...
		if (p_M_Locator_ID != 0)
		{
			sql = new StringBuffer ("UPDATE I_Inventory "
				...
				+ " AND I_IsImported<>'Y'").append (clientCheck);
			no = DB.executeUpdate (get_TrxName(), sql.toString (),p_M_Locator_ID,p_AD_Client_ID);
		...
		try
		{
			...
			while (rs.next())
			{
				...
				if (imp.getLot() != null || imp.getSerNo() != null)
				{
					...
					{
						if (imp.getSerNo().equals("Y")) 
							M_AttributeSetInstance_ID = Integer.parseInt(imp.getLot());
						else
						{
							...
							masi.setDescription();
							//CambioWDiaz Colocar el Costo en Moneda Nacional En la Instancia
							masi.set_Value("PriceActual", rs.getBigDecimal("PriceActual"));
							masi.set_Value("XX_SalePrice", rs.getBigDecimal("XX_SalePrice"));
							masi.save();
