...
public final class ImpFormat
{
	...
	public String[] parseLine (String line, boolean withLabel, boolean trace, boolean ignoreEmpty)
	{
		...
		for (int i = 0; i < m_rows.size(); i++)
		{
			...
			if (row.getColumnName().equalsIgnoreCase("OrgValue")){
				int end = entry.toString().indexOf("'", 10);
				if (end >= 10)
					m_orgValue = entry.substring(10, end).toString();
			}
			
			if (row.getColumnName().equalsIgnoreCase("OrgName")){
				int end = entry.toString().indexOf("'", 9);
				if (end >= 9)
					m_orgName = entry.substring(9, end).toString();
			}
	 ...
	 *	@return -1 if error, 0 if inserted, 1 if merged
	 */
	public int updateDB (Ctx ctx, String line, Trx trx)
	{
	...
	//	log.config( "ImpFormat.updateDB - listSize=" + nodes.length);
		...
		int AD_Org_ID = -1;
		MOrg orgs[] = MOrg.getOfClient(ctx, AD_Client_ID);
		if (m_orgValue!=null && m_orgValue.length()>0)
			for (MOrg org: orgs)
				if (org.getValue().equalsIgnoreCase(m_orgValue)){
					AD_Org_ID = org.getAD_Org_ID();
					break;
				}
		if (AD_Org_ID==-1 && m_orgName!=null && m_orgName.length()>0)
			for (MOrg org: orgs)
				if (org.getValue().equalsIgnoreCase(m_orgValue)){
					AD_Org_ID = org.getAD_Org_ID();
					break;
		...
		if (ID == 0)
		{
			ID = DB.getNextID(ctx, m_tableName, trx);		//	get ID
			sql = new StringBuffer("INSERT INTO ")
				...
				.append(") VALUES (?,?,?,SysDate,?,SysDate,?,'Y')");
			//
			Object[] params = new Object[]{ID,AD_Client_ID,AD_Org_ID,UpdatedBy,UpdatedBy};
		...
		sql.append("IsActive='Y',Processed='N',I_IsImported='N',Updated=SysDate,UpdatedBy=?");
		sql.append(" WHERE ").append(m_tablePK).append("=?");
		//  Update Cmd
		int no = DB.executeUpdate(trx, sql.toString(), new Object[]{UpdatedBy, ID});
		if (no != 1)
		{
