...
/**
 * Detalle de Redistribución de Traspaso
 * @author Jessica Mendoza
 *
 */
public class XX_DetailsRedisTransfers extends CPanel
...
private static final long serialVersionUID = 1L;
	...
	private TitledBorder xBorder =
		new TitledBorder(Msg.getMsg(Env.getCtx(), "XX_DetailRedisTransfers"));
	...
	private void jbInit() throws Exception{
		...
		totalGeneralLabel.setText(Msg.getMsg(Env.getCtx(), "XX_TotalRedisTransfers"));   
	...
	private void tableInitF(){
		...
		m_sql.append("with consecutive1 as " +
					 "(select XX_SalePrice as precio, XX_PriceConsecutive as nro, " +
					 "M_Product_ID as idPro, M_AttributeSetInstance_ID as idInst, XX_UnitPurchasePrice as costo " +
					 "from XX_VMR_PriceConsecutive), " +
					 "consecutive2 as " +
					 "(select XX_SalePrice as precio, XX_PriceConsecutive as nro, " +
					 "M_Product_ID as idPro, M_AttributeSetInstance_ID as idInst, XX_UnitPurchasePrice as costo " +
					 "from XX_VMR_PriceConsecutive) " +
					 "select dep.XX_VMR_Department_ID, dep.value||' '||dep.name, to_char(vor.Created,'dd/MM/yyyy'), " +
					 "vor.XX_OrderBecoCorrelative, pro.value, pro.M_Product_ID, " +
					 "con1.precio as pActual, con1.costo as cActual, con2.precio as pAnterior, con2.costo as cAnterior, " +
					 "sum(vod.XX_ProductQuantity) as cantProduct, sum(vod.XX_SalePrice) as precio, " +
					 "sum(vod.XX_TaxAmount) as precioTax, sum(asi.PriceActual) as costoS, sum(vod.XX_TaxAmount) as costoTax " +
					 "from XX_VMR_Order vor " +
					 "inner join XX_VMR_OrderRequestDetail vod on (vor.XX_VMR_Order_ID = vod.XX_VMR_Order_ID) " +
					 "inner join M_Warehouse war on (vor.M_Warehouse_ID = war.M_Warehouse_ID) " +
					 "inner join C_Order ord on (ord.C_Order_ID = vor.C_Order_ID) " +
					 "inner join XX_VMR_Department dep on (ord.XX_VMR_Department_ID = dep.XX_VMR_Department_ID) " +
					 "left join M_AttributeSetInstance asi on (asi.M_AttributeSetInstance_ID = vod.XX_ProductBatch_ID) " +
					 "inner join M_Product pro on (vod.M_Product_ID = pro.M_Product_ID) " +
					 ...
					 "inner join C_BPartner par on (ord.C_BPartner_ID = par.C_BPartner_ID) " +
					 "where vor.XX_OrderRequestStatus in ('TI') " +
					 "and war.value <> 001 ");
		m_groupBy = "group by dep.XX_VMR_Department_ID, dep.value, dep.name, vor.Created, vor.XX_OrderBecoCorrelative, " +
					"pro.value, con1.precio, con1.costo, pro.M_Product_ID, con2.precio, con2.costo ";
		...
		if (fecha != null){
			...
			m_sql.append("and to_char(vor.XX_DateStatusOnStore,'yyyy') = ").append(cal.get(GregorianCalendar.YEAR)).append(" ");
			m_sql.append("and to_char(vor.XX_DateStatusOnStore,'MM') = ").append(cal.get(GregorianCalendar.MONTH)+1).append(" ");
		}
