	...
	implements ActionListener, TableModelListener
{
	 ...
	 *  @return JDialog
	 */
	public static VCreateFrom create (GridTab mTab, int invoiceID)
	{
	...
	/** Invoice ID               */
	protected static int               c_InvoiceID;
	/** Window No               */
	...
	public static ArrayList< NamePair > getOrders( Ctx ctx, int C_BPartner_ID, boolean isReturnTrx, boolean forInvoice, int winNo )
	{
		...
		/****Jessica Mendoza****/
		//  Valida que el tipo de factura haga merge con el tipo de O/C
		MInvoice invoice = new MInvoice(Env.getCtx(), ctx.getContextAsInt( winNo, "C_Invoice_ID"), null);
		String column2 = "", column3 = "";
		if (invoice.get_Value("XX_InvoiceType").equals("I")){
			column2 = "o.XX_POType = 'POM' and ";
		}else if (invoice.get_Value("XX_InvoiceType").equals("A")){
			column3 = "o.XX_POType = 'POA' and o.XX_PurchaseType in ('FA','SU') and ";
		}else
			column3 = "o.XX_POType = 'POA' and o.XX_PurchaseType = 'SE' and ";
		...
		StringBuffer sql1 = new StringBuffer( "SELECT o.C_Order_ID," ).append( display ).append(
				" FROM C_Order o " + "WHERE " +
						"o.XX_InvoicingStatus <> 'AP' and ").append(column2).append("" +
						"(o.XX_OrderStatus = 'RE' or o.XX_OrderStatus = 'CH') and ").append(column3).append("" +   // Realizado para que no se vean mas las ordenes de compra para un proveedor si ya fue aprobada en Facturacion (WDiaz)
	...
	public static Vector< Vector< Object > > getOrderData( Ctx ctx, int C_Order_ID, boolean forInvoice)
	{
		...
		StringBuffer sql = new StringBuffer("SELECT "
			...
			+ "(Select value from XX_VMR_VendorProdRef a where a.XX_VMR_VendorProdRef_ID = p.XX_VMR_VendorProdRef_ID) refvalue "					
			+ "FROM C_OrderLine l"
		...
		//
		sql.append(" WHERE l.PROCESSED = 'Y' AND l.C_Order_ID=? AND l.C_OrderLine_ID NOT IN (SELECT ilAux.C_OrderLine_ID FROM C_InvoiceLine ilAux, C_Invoice iAux  WHERE ilAux.C_Invoice_ID = iAux.C_Invoice_ID and  iAux.C_ORDER_ID = l.C_Order_ID and C_OrderLine_ID is not null and iAux.DocStatus not in ('VO', 'RE'))"			//	#1
		...
		try
		{
			...
			while (rs.next())
			{
				...
				if (rs.getInt(10) == 0){ //Modificado por Jessica Mendoza
					...
					pp = new KeyNamePair(rs.getInt(10), rs.getString("refvalue").trim()); //WDiaz // 6
					line.add(pp);
				}
								// Referencia WDiaz
