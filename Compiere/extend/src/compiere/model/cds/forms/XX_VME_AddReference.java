...
public class XX_VME_AddReference extends XX_ProductFilter{
	...
	**/
	...
	protected void fillSecondTable(){		
		...
		////
		/*miniTable2.getColumnModel().getColumn(4).setPreferredWidth(200); // Referencia
		miniTable2.getColumnModel().getColumn(1).setPreferredWidth(200); // Departamento
		miniTable2.getColumnModel().getColumn(2).setPreferredWidth(200); // Linea
		miniTable2.getColumnModel().getColumn(3).setPreferredWidth(200); // Seccion
		miniTable2.getColumnModel().getColumn(5).setPreferredWidth(60);  // Manual
		miniTable2.getColumnModel().getColumn(6).setPreferredWidth(60);  // Cantidad
		miniTable2.getColumnModel().getColumn(7).setPreferredWidth(60);  // Mantener
		*/
	...
	public void getReferences(boolean product){
			...
			for (int fila = 0; fila < products; fila++) {
				...
				if(reference == 0) {
					...
					XX_VME_GeneralFunctions.createElemRef(elemento, 					// elemento 
							...
							quantities.get(0),										// cantidad
							prod.getXX_VMR_VendorProdRef_ID(),						// referencia
							prod.getXX_VMR_Category_ID(), 							// categoria
							"P", 													// tipo
							false,													// manual o no
							false,													// mantener
							prod.get_ID(),											// producto
		...
		// La búsqueda refleja resultados a nivel de referencia
			...
			for (int fila = 0; fila < filas2; fila++) {
				//System.out.println(((KeyNamePair)miniTable2.getModel().getValueAt(fila, 0)).getKey());
				...
				if(reference == 0){
					XX_VME_GeneralFunctions.createElemRef(elemento,							// elemento 
						action, 															// accion de mercadeo
						(BigDecimal)miniTable2.getModel().getValueAt(fila, 6), 				// cantidad
						((KeyNamePair)miniTable2.getModel().getValueAt(fila, 0)).getKey(),	// referencia
						((KeyNamePair)miniTable2.getModel().getValueAt(fila, 1)).getKey(), 	// categoria
						"R",																// tipo
						(Boolean)miniTable2.getModel().getValueAt(fila, 5), 				// manual o no
						(Boolean)miniTable2.getModel().getValueAt(fila, 7),					// mantener
						0,																	// producto
						false);
				}
				// La referencia ya existía, se actualiza la cantidad indepabis
					...
					//TODO redistribuir entre sus productos la nueva cantidad
