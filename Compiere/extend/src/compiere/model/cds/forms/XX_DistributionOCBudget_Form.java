 ...
 *  @version    
 */
public class XX_DistributionOCBudget_Form extends CPanel
	...
	private X_XX_VMR_DistributionHeader header;	
	...
	private void dynInit() {	
		...
		header = new X_XX_VMR_DistributionHeader(Env.getCtx(), Env.getCtx().getContextAsInt("#XX_VMR_BUD_DistributionHeader_ID"), null);
		Env.getCtx().remove("#XX_VMR_BUD_DistributionHeader_ID");
	...
	private void gettingData(){
		...
		try 
		{
			pstmt = DB.prepareStatement(sql_w, null);
	...
	private void getDistribByBudget(){
		...
		int dept = order.getXX_Department();
		String sql = 
			"SELECT wh.M_WAREHOUSE_ID, SUM(ip.XX_SALESAMOUNTBUD) FROM XX_VMR_PRLD01 ip" 
		...
		try{
			...
			{	
				...
				budget.add(rs.getInt(2));
				totalBudget +=  rs.getInt(2);
	...
	private void savingData(){		
		...
		boolean it_work = XX_PercentualDistribution.applyPOPercentualDistribution(
				header, percentages, warehouse_codes, m_WindowNo , m_frame, 0);
		if (it_work ) dispose();
