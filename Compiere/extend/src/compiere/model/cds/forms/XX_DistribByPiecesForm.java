 ...
 */
public class XX_DistribByPiecesForm extends CPanel
	implements FormPanel, ActionListener, TableModelListener, ListSelectionListener, KeyListener, MouseListener
{
	...
	public void init (int WindowNo, FormFrame frame)
	{
		...
		Ctx aux = Env.getCtx();		
		int header_id = aux.getContextAsInt("#XX_VMR_QTY_DistributionDetail_ID");		
		mostrarPiezas = (aux.getContextAsInt("#XX_VMR_QTY_ShowPercentages")) == 0;		
		trans = Trx.get("XX_CHANGE_DISTRIB_PRODUCTQTY");
			//Remove the no longer necessary items on the context				
		aux.remove("#XX_VMR_QTY_DistributionDetail_ID");		
		if (!mostrarPiezas) 
		...
		detail = 
			new X_XX_VMR_DistributionDetail(aux, header_id , trans);
		try
		{			
			jbInit();			
			dynInit();
	...
	private void dynInit()	
		...
		String sql_warehouses = " SELECT M_WAREHOUSE_ID, VALUE||'-'||NAME FROM M_WAREHOUSE WHERE ISACTIVE = 'Y' AND XX_NotReceiveOrder = 'N' " +
				" AND (XX_ISSTORE = 'Y' OR M_WAREHOUSE_ID = "+Utilities.obtenerDistribucionCD(detail.getXX_VMR_DistributionHeader_ID())+") ";				
		sql_warehouses = MRole.getDefault().addAccessSQL(
			sql_warehouses, "", MRole.SQL_NOTQUALIFIED, MRole.SQL_RO)
				+ " ORDER BY VALUE";
	...
	public void fillMatchTable( ) {
		...
		String sql =		
			" SELECT P.XX_VMR_DISTRIBPRODUCTDETAIL_ID, P.M_AttributeSetInstance_ID, PR.M_PRODUCT_ID, PR.VALUE||'-'||PR.NAME as PRODUCT, " +
			" XX_QUANTITY, P.XX_USESIZECURVE AS CURVE, RE.DESCRIPTION AS REF" +
			" FROM XX_VMR_DISTRIBPRODUCTDETAIL P JOIN M_PRODUCT PR ON (P.M_PRODUCT_ID = PR.M_PRODUCT_ID)" +
			" JOIN XX_VMR_VENDORPRODREF RE ON (PR.XX_VMR_VENDORPRODREF_ID = RE.XX_VMR_VENDORPRODREF_ID)  " +
			" WHERE P.XX_VMR_DISTRIBUTIONDETAIL_ID = ? " +
			" ORDER BY RE.DESCRIPTION, PR.VALUE";
		try
		{
			int row = 0;
			PreparedStatement pstmt = DB.prepareStatement(sql, null);			
			pstmt.setInt(1, detail.get_ID());			
			...
			while (rs.next())
			{
				...
				//1- product distrib
				IDColumn idcolumn = new IDColumn(rs.getInt("XX_VMR_DISTRIBPRODUCTDETAIL_ID"));
				...
				if (mostrarPiezas)
					productTable.setValueAt(rs.getInt("XX_QUANTITY"), row, M_STORE + 2*warehouses_number);
				else 
					productTable.setValueAt(Env.ONEHUNDRED, row, M_STORE + 2*warehouses_number);
				boolean used_size_curve = false;
				if (rs.getString("CURVE").equals("Y")) used_size_curve = true;
					
				productTable.setValueAt(used_size_curve, row, M_HASSIZECURVE);				
				product_qtys.put(rs.getInt(1), new Hashtable<Integer, Integer>());				

				//Ver si hay inconsistencias y avisar de ellas
				if (loadMatchedToTable(rs.getInt("XX_VMR_DISTRIBPRODUCTDETAIL_ID"), row, 
						used_size_curve, rs.getDouble("XX_QUANTITY"))) {					
	...
	public boolean loadMatchedToTable(int po_product_id, int row , boolean size, double cantidad) {
		...
		String sql = "SELECT  M_WAREHOUSE_ID ID, XX_QUANTITY QTY , XX_SIZECURVECOMPLIANCE SIZEC " + 
			" FROM XX_VMR_DISTRIBPRODPERSTORE " +  
			" WHERE XX_VMR_DISTRIBPRODUCTDETAIL_ID = " + po_product_id;			
		try {
			PreparedStatement pstmt = DB.prepareStatement(sql, null);			
			ResultSet rs = pstmt.executeQuery();
	...
	private void cmd_process() {		
		...
		if (ok_to_distribute) {
			...
			X_XX_VMR_DistributionHeader header_modify = 
				new X_XX_VMR_DistributionHeader(Env.getCtx(), detail.getXX_VMR_DistributionHeader_ID(), trans);
			...
			for (int i = 0 ; i < productTable.getRowCount(); i++) {	
				...
				row = i;
				try {
					String sql = "DELETE FROM XX_VMR_DistribProdPerStore WHERE XX_VMR_DistribProductDetail_ID = " + idcolumn.getRecord_ID();
					...
					for (int j = 0; j < warehouses_number; j++) {
						...
						X_XX_VMR_DistribProdPerStore distribution = 
							new X_XX_VMR_DistribProdPerStore(Env.getCtx(), 0, trans);
											
						//Store the object on the data base
						distribution.setXX_VMR_DistribProductDetail_ID(idcolumn.getRecord_ID());
						distribution.setM_Warehouse_ID(warehouses_codes.elementAt(j));
						distribution.setXX_SizeCurveCompliance(false);
						distribution.setXX_Quantity(qty);
						if (qty != 0.0) {
							distribution.save();						
						}
					}					
