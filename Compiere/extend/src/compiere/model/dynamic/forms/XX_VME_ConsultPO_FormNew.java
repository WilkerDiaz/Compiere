...
public class XX_VME_ConsultPO_FormNew extends CPanel implements FormPanel,
	...
	/** Logger */
	static CLogger log = CLogger.getCLogger(XX_VME_ConsultPO_FormNew.class);
	...
	void dynType() {
		...
		String sql = " SELECT DISTINCT L.NAME, L.AD_REF_LIST_ID ID " +
					...
					" AND R.AD_CLIENT_ID = " + Env.getCtx().getAD_Client_ID() +
					" AND L.ISACTIVE='Y' " +
					" AND VALUE IN ('P', 'G')";
		//System.out.println("SQL OrderStatus: "+sql);
...
//						categories.add(dept.getXX_VMR_Category_ID());
						productos.add(0);
						order.add(corderid);
						pedido.add(pedidoid);
					}
		...
		String Elemenefs = "UPDATE XX_VME_ELEMENTS SET  XX_VME_QtyRefAssociated = " + 
						element.getXX_VME_QtyRefAssociated().add(new BigDecimal(referencias.size())) +
