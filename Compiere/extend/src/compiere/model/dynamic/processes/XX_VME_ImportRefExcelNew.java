 ...
 *  
 * **/
public class XX_VME_ImportRefExcelNew extends SvrProcess {
...
//	private int ano = cal.get(Calendar.YEAR);
	...
	protected String doIt() throws Exception {
		...
		Vector<Integer> referencias = new Vector<Integer>();// Elemento, Folleto, pagina y accion
			...
			if(sheet.getColumns() == 7 && sheet.getRows() > 1){
				...
				if(lectura){
					...
					for (int i = 1; i < sheet.getRows(); i++) {
						...
						if(categoryID == 0){
							//ADialog.error(1, new Container(), Msg.translate( Env.getCtx(), "XX_CatError" ));*/		
							//break;
							continue;
						} // cat
						else {
						...
						if(departmentID == 0){
							// ADialog.info(1, new Container(), Msg.translate( Env.getCtx(), "XX_DepDefError") );	
							continue;
						} // dep
						...
						if(lineID == 0){
							//ADialog.error(1, new Container(), Msg.translate( Env.getCtx(), "XX_LinError" ));		
							//break;
							continue;
						} // lin
						...
						if(sectionID == 0){
							//ADialog.error(1, new Container(),Msg.translate( Env.getCtx(), "XX_SecError" ));	
							//break;
							continue;
						} // sec
						...
						if(referenceValue.equals("")){
							...
							for(int j = 0; j < references.size(); j++){
								...
								reference = XX_VME_GeneralFunctions.obtainReference(references.get(j), elemento);
								...
								if(reference == 0) {
									...
									referencias.add(references.get(j));
//									categories.add(categoryID);
									manualProd.add(manual);
									manualV.add(manual);
									mantenerV.add(mantener.equalsIgnoreCase("Y"));
					...
					// Se asocian las referencias y productos al elemento
					...
					// Se actualiza la cantidad de las referencias asociadas al elmento 
	...
	private int getLineID(String line, int departmentID){
		...
		try{
			...
			try { rsQueryLin.close(); } 
			catch (SQLException e) { e.printStackTrace(); }
			try { psQueryLin.close(); } 
			catch (SQLException e) { e.printStackTrace(); }
		}
		return linID;
	} // getLineID
